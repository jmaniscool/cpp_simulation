...
int main() {
	...
	double area = 1e2; //size of the simulation (number of cells)
	...
	const char* initial_state_filename = "large_steadystate.txt"; //file which holds large initial state to sample from.

	return_4tuple q = cpp_sim(area, time_max, weakening, consv, rate, w, a, modulus, to_write, to_write_failures, is_forcerate, initial_state_filename);
	return 0;
}